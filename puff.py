...
class Puff():
    def __init__(self, hr, qm, tipo_atm, v_vento):
    ...
    # Valores de sigma
    def sigma_puff(self, atm, dist = 1):
        '''
    ...
    def pasquill_gifford_15(self, x, y, z, t):
        ...
        dist = self.distancia(x, y, z) # Distancia entre a fonte e o local analisado
...
if __name__ == '__main__':
    ...
    exp_y = exp((-pow(0 / 150 , 2) / 2))
    exp_z = exp(-pow((25 - 50) / 100, 2) / 2) + exp(-pow((25 + 50) / 100, 2) / 2)
    exp_xut = exp(-pow((500 - 10 * 60) / 200, 2) / 2)
    sub = (pow((2*pi), 3/2)) * 200 * 150 * 100

    concent = (100 * exp_y * exp_z * exp_xut) / sub
